...
window.addEventListener('DOMContentLoaded', () => {
    ...
    axios.get('http://18.208.188.31:3000/getProducts').then(response => {
        renderPage(response);
    })

    axios.get('http://18.208.188.31:3000/getCart').then(cartItems => {
...
parentContainer.addEventListener('click', (e) => {
    ...
    if (e.target.className == 'shop-item-button') {
        ...
        axios.post(`http://18.208.188.31:3000/addCart`, { "productId": productId }).then((data) => {
            if (data.status === 200) {
    ...
    if (e.target.className == 'purchase-btn') {
        ...
        axios.get("http://18.208.188.31:3000/purchase")
            .then(response => {
...
function pagination(page) {
    axios.get(`http://18.208.188.31:3000/getProducts?page=${page}`)
        .then(response => {
            renderPage(response);
        })
        .catch(err => {
